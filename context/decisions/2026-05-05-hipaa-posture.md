...
1. ✅ **Privacy Policy rewrite drafted (2026-05-12).** Replaced the "therapeutic data as sensitive health information" framing with "personal wellness data." Added a new Section 1 "Not a HIPAA-Covered Service" stating explicitly that Huxley is not a covered entity / business associate / healthcare provider, that data recorded in the App is not PHI, and that no clinician sees in-app content through Huxley. Reframed the "Therapeutic & Integration Data" subheading to "Personal Wellness & Reflection Data" and reworded the data-storage and data-retention bullets accordingly. Sections renumbered (now 12 sections; LAST_UPDATED → May 12, 2026). **Still awaiting legal reviewer (BUG-308).**
2. ✅ **Terms of Service rewrite drafted (2026-05-12).** Section 1 now explicitly positions Huxley as a consumer wellness product and disclaims any clinician–patient relationship. Section 2 ("Not Medical, Psychiatric, or Therapeutic Care") tightened: AI guide reframed as "reflection companion, not a therapist or clinician," added an explicit "Huxley does not create, store, or transmit a clinical record" bullet, broadened the informational-purposes bullet to cover contributor submissions. New Section 3 "Not a HIPAA-Covered Service" mirrors the Privacy Policy section: not a covered entity, OS-share-sheet flow described as user-initiated and outside-Huxley, contributor educational-content disclaimer. Sections renumbered (now 15 sections; LAST_UPDATED → May 12, 2026). **Still awaiting legal reviewer (BUG-308).**
...
7. ✅ **Done (2026-05-12):** Added `screens/NonClinicalDisclosureScreen.js` — a one-time post-onboarding disclosure that surfaces the wellness-vs-clinical, no-HIPAA, and crisis-services framing in three short cards. Includes an "Are you a healthcare practitioner?" yes/no question; selecting Yes expands an extended note that articulates the ADR boundary directly to that user (personal use OK, recommending to clients OK with consent, but no in-app HIPAA-covered client workflow, no BAA, OS-share-sheet handoff is outside Huxley). Acknowledgement is required to proceed (no skip). Persisted to AsyncStorage under a versioned key (`huxley:disclosure_acknowledged_v1`) so future material changes can re-prompt. Practitioner self-ID also persisted (`huxley:is_practitioner`) for future use. App.js gates render after onboarding and before main app routes.
