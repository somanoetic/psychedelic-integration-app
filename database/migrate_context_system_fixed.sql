-- ============================================================================
-- CONTEXT SYSTEM MIGRATION (FIXED)
-- ============================================================================
...
-- DROP TABLE IF EXISTS ifs_session_history CASCADE;
-- DROP TABLE IF EXISTS ifs_parts_inventory CASCADE;
-- DROP TABLE IF EXISTS polyvagal_patterns CASCADE;
-- DROP TABLE IF EXISTS nervous_system_context CASCADE;
...
CREATE TABLE IF NOT EXISTS ifs_parts_inventory (
  ...
  -- Constraints
  CONSTRAINT valid_part_role CHECK (part_role IN ('manager', 'firefighter', 'exile')),
  CONSTRAINT valid_unburdening_method CHECK (unburdening_method IS NULL OR unburdening_method IN ('light', 'water', 'fire', 'earth', 'wind', 'other')),
  CONSTRAINT valid_work_phase CHECK (work_phase IN ('discovery', 'accessing', 'unburdening', 'integrated'))
);
...
CREATE TABLE IF NOT EXISTS ifs_session_history (
  ...
  -- Constraints
  CONSTRAINT valid_session_type CHECK (session_type IN ('discovery', 'check_in', 'exile_access', 'unburdening', 'follow_up'))
);
...
CREATE TABLE IF NOT EXISTS polyvagal_patterns (
  ...
  -- Constraints
  CONSTRAINT valid_regulation_capacity CHECK (regulation_capacity IS NULL OR regulation_capacity IN ('building', 'developing', 'moderate', 'strong')),
  CONSTRAINT valid_awareness_level CHECK (awareness_level IS NULL OR awareness_level IN ('beginning', 'developing', 'practiced', 'embodied'))
);
...
CREATE TABLE IF NOT EXISTS nervous_system_context (
  ...
  -- Constraints
  CONSTRAINT valid_ns_regulation_capacity CHECK (regulation_capacity IN ('building', 'developing', 'moderate', 'strong')),
  CONSTRAINT valid_ns_awareness_level CHECK (awareness_level IN ('beginning', 'developing', 'practiced', 'embodied'))
);
