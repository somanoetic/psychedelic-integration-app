...
Remember: You're facilitating THEIR wisdom, not teaching them yours.`;
  ...
  async getAIResponse(userMessage) {
    ...
    // Fetch RAG context for regulation resources knowledge
    let ragContext = '';
    try {
      ragContext = await ragService.getContextForPrompt(userMessage, {
        maxResults: 3, maxTokens: 1500, threshold: 0.4, categories: ['polyvagal', 'somatic'],
      });
    ...
    const response = await fetch(CLAUDE_API_URL, {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 600,
        system: this.getSystemPrompt() + `\n\nCurrent Phase: ${this.phase}` + ragContext,
