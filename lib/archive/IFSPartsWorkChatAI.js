...
const IFSPartsWorkChatAI = ({ onComplete, onSkip }) => {
  ...
  // Session data tracked locally (not in unified service)
  const sessionDataRef = useRef({
    targetPart: '', location: '', description: '',
    partRole: '', selfEnergy: '', partFears: '', partResponse: ''
  });
...
All parts have positive intentions, even when their methods cause problems. This work is about building relationship with them.`;
  ...
  const handleOptionSelect = async (option) => {
    ...
    if (currentPhase === 'intro') {
      if (option === 'Begin Session') {
        ...
        setTimeout(async () => {
          const aiResponse = await huxleyService.chat("I'd like to begin working with a part.", { phase: 'find' });
          setIsTyping(false);
  ...
  const handleSendMessage = async (messageOverride = null) => {
    ...
    try {
      ...
      const aiResponse = await huxleyService.chat(message, { phase: currentPhase });
      ...
      advancePhaseIfNeeded(message, aiResponse.message);
  ...
  const updateSessionDataFromPhase = (message) => {
    ...
    switch (currentPhase) {
      case 'find':
        sessionDataRef.current.targetPart = message;
        break;
      case 'findLocation':
        sessionDataRef.current.location = message;
        break;
      case 'focus':
        sessionDataRef.current.description = message;
        break;
      case 'fleshOut':
        sessionDataRef.current.partRole = message;
        break;
      case 'feelToward':
        sessionDataRef.current.selfEnergy = message;
        break;
      case 'befriend':
        sessionDataRef.current.partResponse = message;
        break;
      case 'fears':
        sessionDataRef.current.partFears = message;
        break;
    }
  };
