...
Remember: You're helping them see their beliefs clearly and compassionately shift what no longer serves them.`;
  ...
  async getAIResponse(userMessage) {
    ...
    // Fetch RAG context for core beliefs knowledge
    let ragContext = '';
    try {
      ragContext = await ragService.getContextForPrompt(userMessage, {
        maxResults: 3, maxTokens: 1500, threshold: 0.4, categories: ['beliefs', 'cbt-act'],
      });
    ...
    const response = await fetch(CLAUDE_API_URL, {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 800,
        system: this.getSystemPrompt(results) + ragContext,
