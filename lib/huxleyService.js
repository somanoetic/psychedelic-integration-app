...
class HuxleyService {
  constructor() {
    ...
    this.therapeuticState = {
      ...
      // Coarse labels only — see the therapeutic_context migration for why.
      // Read by every mode AND the main routing chat, so treat as user-facing.
      majorEvents: [],
      sessionSummary: null,
      lastSessionAt: null,
    };
  ...
  async chat(message, options = {}) {
    ...
    try {
      ...
      if (__DEV__) {
        ...
        // build= splits into asm= (pure synchronous string assembly) + ragWait=
        // (time BLOCKED awaiting RAG at the tail). If asm= is small and ragWait=
        // dominates, "slow build" is really just RAG on the critical path, NOT
        // string work — don't optimize assembly. If asm= is large, the string
        // building itself is the ttft cost worth chasing.
        const buildSplit = ` (asm=${this._lastAsmMs ?? '?'}ms ragWait=${this._lastRagWaitMs ?? '?'}ms)`;
        console.log(
          `[Huxley PERF] mode=${mode} rag=${this._lastRagMs ?? '?'}ms${reuseTag}${ragSplit}${ragTag} ` +
          `build=${buildMs}ms${buildSplit} ${ttftTag}claude=${claudeMs}ms total=${Date.now() - startTime}ms ` +
  ...
  _startRagRetrieval(modeConfig) {
    ...
    // Expand the EMBED query with mode-specific clinical framing (see
    // _expandRagQuery). lastUserMsg (raw) is still what the reuse closeness
    // check compared above; only the text we embed is framed.
    const expandedQuery = this._expandRagQuery(lastUserMsg, modeConfig);
    const ragFetch = ragService
      .getContextForPrompt(expandedQuery, {
        maxResults: 3,
        maxTokens: 1500,
  ...
  prefetchRag(draftText, options = {}) {
    ...
    // Expand the EMBED query the SAME way _startRagRetrieval does, so the
    // prefetched retrieval matches what a fresh send-turn search would return
    // and the reuse is valid. Store the RAW `query` (not expanded) in
    // _ragPrefetch — _ragQueryIsClose compares raw user text on both sides.
    const expandedQuery = this._expandRagQuery(query, modeConfig);
   ...
   */
  /**
   * Query expansion for RAG. The knowledge corpus is dense clinical/academic
   * prose (IFS parts language, polyvagal states, somatics). Raw everyday-language
   * utterances ("I don't want to be a doctor anymore", "I'm not happy at work")
   * embed too far from that register and score below the 0.30 threshold, so they
   * return 0 results even when the corpus has on-topic material. This appends a
   * small set of mode-specific framing terms to the text we EMBED — never to the
   * user-visible message — nudging the query vector toward the corpus register.
   *
   * Corpus-verified via knowledge-base/rag/search.py (see handoffs/rag-query-expansion.md):
   * with ifs framing, "ER doctor" went 0 -> 46%, "trapped in career" 35 -> 52%,
   * and already-good jargon/somatic queries held or improved (no degradation).
   * Per-mode vocab lifts each mode's own corpus: nervous_system 41 -> 48%,
   * core_beliefs 39 -> 51% (and "everyone always leaves me" 0 -> 38%),
   * intention 41 -> 61%.
   *
   * Deterministic, zero-cost, zero-latency — it's pure string concatenation, so
   * it composes cleanly with the prefetch-while-typing path (no extra round-trip
   * to hide). Modes with ragCategories:null (general, journal, integration) get
   * ragFraming:null and are returned unchanged — they span the whole corpus, so
   * there's no single register to bias toward.
   *
   * IMPORTANT: expansion is applied ONLY to the getContextForPrompt query text.
   * The reuse closeness check (_ragQueryIsClose) still compares RAW user text —
   * if it saw expanded text, every query would carry identical framing and look
   * falsely "close," corrupting reuse. Keep raw text for closeness, expanded text
   * for embedding.
   */
  _expandRagQuery(query, modeConfig) {
    const framing = modeConfig && modeConfig.ragFraming;
    if (!framing || !framing.length) return query;
    const q = (query || '').trim();
    if (!q) return q;
    // "<query>. <term>, <term>, ..." — matches the format proven in the harness.
    return `${q}. ${framing.join(', ')}`;
  }
  ...
  async _buildSystemPrompt(mode, phase, modeContext, ragPromise) {
    // Mark the start of synchronous assembly so the tail can compute asm= (pure
    // string work) separately from ragWait= (blocking on RAG). See PERF log.
...
- Brief, warm, plain. One question at a time. Keep checking in on safety.`);
    ...
    // At this point ALL synchronous assembly is done; the only thing left is the
    // RAG await. Capture the split so the PERF line can separate pure string
    // assembly (asm=) from time BLOCKED waiting on RAG at the tail (ragWait=).
    // This disambiguates build=: previously build= folded both together, so a
    // "slow build" was indistinguishable from "RAG hadn't finished yet."
    const asmMs = Date.now() - this._buildStartedAt;
    ...
    if (ragPromise) {
      ...
      ragWaitMs = Date.now() - waitStart;
      if (ragContext) tailParts.push(ragContext);
    }

    // Expose the split to the PERF logger in chat(). Instance fields (not a
    // return value) so the existing destructure of { systemBlocks, volatileTail }
    // stays untouched.
    this._lastAsmMs = asmMs;
    this._lastRagWaitMs = ragWaitMs;
  ...
  /**
   * Human-friendly recency for the "where you left off" header, so Huxley can
   * pitch continuity correctly — picking up a thread from yesterday reads very
   * differently from picking one up after two months.
   * @param {string|null} iso
   * @returns {string} e.g. "earlier today", "3 weeks ago", or '' if unknown
   */
  ...
  _buildSharedContext() {
    ...
    // Significant life events (coarse labels only — see migration).
    // The surfacing rule matters as much as the data: the user did not
    // necessarily raise this HERE, and material appearing where they didn't put
    // it can feel surveilled rather than held.
    if (this.therapeuticState.majorEvents?.length > 0) {
      lines.push(`\nSIGNIFICANT EVENTS IN THEIR LIFE:`);
      this.therapeuticState.majorEvents.forEach(e => {
        lines.push(`- ${e.label}${e.surfaced_at ? ` (came up ${e.surfaced_at})` : ''}`);
      });
      lines.push(
        'HOW TO USE THESE: primarily so you are not tone-deaf — do not open breezily ' +
        'if something heavy is live. You MAY reference one gently and briefly if it fits ' +
        'naturally ("last time your dad came up — where are you with that?"). Then follow ' +
        'their lead: if they do not pick it up, let it go completely and do not raise it again. ' +
        'Never interrogate, never stack multiple events, never treat these as an agenda to ' +
        'work through. You have only a short label and no detail — do not invent specifics, ' +
        'and do not imply you remember more than the label says.'
      );
    }

    // Where the last conversation left off.
    if (this.therapeuticState.sessionSummary) {
      const when = this._describeRecency(this.therapeuticState.lastSessionAt);
      lines.push(`\nWHERE YOU LEFT OFF${when ? ` (${when})` : ''}:`);
      lines.push(this.therapeuticState.sessionSummary);
      lines.push(
        'This is your own recap of the previous conversation, not something they said ' +
        'just now. Use it for continuity; do not recite it back to them.'
      );
    }
...
- parts CHANNEL FIELDS (IFS work — include any that the user actually revealed this turn, omit the rest; never guess or fill in): "location" (where in/around the body), "sensation" (what the sensation is like), "behavior" (what the part makes them do or avoid), "feelings" (the emotion it carries), "thoughts" (its thoughts / the actual words it says), "voice" (its tone or whose voice it sounds like), "memory" (a memory or scene it's tied to, or when it first appeared), "appearance" (image, figure, colors, textures), "age" (how old it seems), "urge" (what it wants to do right now). These are how a part gets differentiated from other parts, and they matter as much as role — record whichever channel the user spoke in, not just the somatic one.
...
- majorEvents: Significant things happening in this person's LIFE that you would look tone-deaf for having forgotten next time — a death, a diagnosis, a birth, a breakup, job loss, a trauma that surfaced. NOT every difficult feeling; themes already cover those. Bar: would it be jarring to open breezily next session without knowing this?
- majorEvents is a LABEL ONLY — a short neutral phrase like "grief around father's death" or "recent ADHD diagnosis". Deliberately coarse. Do NOT include what they disclosed in detail, quotes, circumstances, names of other people, or any clinical interpretation. One short phrase, nothing more. Omit or use [] on turns where no significant event surfaced, which is most turns.
- sessionSummary: A rolling 2-3 sentence recap of where THIS conversation has got to — what they came in with, what opened up, where you left it. Written for your future self opening the next session. Rewrite it fresh each turn (this field is NOT delta — send the whole current summary). Plain, factual, no interpretation.
- Always include the block, even if all values are null/empty.`;
  ...
  _updateTherapeuticState(data) {
    ...
    // Merge major events (deduplicate on label).
    // Defensive: the model is told to send {label} only, but we rebuild each
    // entry field-by-field rather than spreading, so any extra key it invents
    // (detail, quote, interpretation...) is dropped rather than persisted. The
    // coarse-grain guarantee is enforced HERE, not just in the prompt.
    if (data.majorEvents && Array.isArray(data.majorEvents)) {
      const existing = new Set(
        this.therapeuticState.majorEvents.map(e => (e.label || '').toLowerCase())
      );
      for (const raw of data.majorEvents) {
        ...
        this.therapeuticState.majorEvents.push({
          label: trimmed.slice(0, 120), // hard cap: a label, never a narrative
          surfaced_at: new Date().toISOString().slice(0, 10),
          mode: this.currentMode,
        });
        existing.add(trimmed.toLowerCase());
      }
      // Keep the most recent 15 (FIFO), mirroring the 20-theme cap. Bounded so
      // the volatile tail can't grow without limit — prompt bloat costs latency.
      if (this.therapeuticState.majorEvents.length > 15) {
        this.therapeuticState.majorEvents = this.therapeuticState.majorEvents.slice(-15);
      }
    }

    // Rolling session summary — full overwrite, not a delta (see instructions).
    if (typeof data.sessionSummary === 'string' && data.sessionSummary.trim()) {
      this.therapeuticState.sessionSummary = data.sessionSummary.trim().slice(0, 600);
      this.therapeuticState.lastSessionAt = new Date().toISOString();
    }
  }
  ...
  async _loadPersistedContext(userId) {
    ...
    try {
      ...
      if (data) {
        ...
        this.therapeuticState.majorEvents = data.major_events || [];
        this.therapeuticState.sessionSummary = data.session_summary || null;
        this.therapeuticState.lastSessionAt = data.last_session_at || null;
        ...
        console.log('[Huxley] Loaded persisted context:', {
          ...
          events: this.therapeuticState.majorEvents.length,
          hasSummary: !!this.therapeuticState.sessionSummary,
        });
      }
    } catch (err) {
  ...
  async _savePersistedContext(userId) {
    ...
    const contextData = {
      ...
      major_events: this.therapeuticState.majorEvents,
      session_summary: this.therapeuticState.sessionSummary,
      last_session_at: this.therapeuticState.lastSessionAt,
  ...
  /**
   * Rehydrate conversation history from a persisted transcript.
   *
   * Used when resuming an interrupted session: the mode handler's own state
   * says WHERE we are, but without this the model has no memory of what was
   * actually said and will repeat itself. Entries are tagged with the current
   * mode/phase so the existing history filters treat them like live turns.
   *
