...
// self-contained and work without it. Measured steady-state RAG latency is
// ~1.5s per turn (OpenAI embedding call + pgvector search), which sits IN SERIES
// ahead of the Claude call. If retrieval is slow, we'd rather ship the turn
// without a knowledge chunk than make the user wait. This budget caps how long
// chat() will block on RAG before proceeding with no RAG context this turn.
// The underlying fetch is not cancelled (it may still populate the RAG cache for
// a later turn) — we just stop waiting on it.
//
// Set as a SAFETY NET, not a kill switch. Measured steady-state RAG is ~1.5s, so
// this sits above that: normal turns still get RAG, and the cap only fires on
// pathological slowness (cold start, network stall). Speeding up RAG itself is
// tracked separately — when that lands, drop this to ~800ms and RAG will fit
    ...
    try {
      ...
      if (__DEV__) {
        ...
        const ragSplit = st
          ...
          : '';
        console.log(
          `[Huxley PERF] mode=${mode} rag=${this._lastRagMs ?? '?'}ms${ragSplit}${ragTag} ` +
          `build=${buildMs}ms claude=${claudeMs}ms total=${Date.now() - startTime}ms ` +
