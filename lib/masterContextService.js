...
class MasterContextService {
  ...
  async getMasterContext(userId, options = {}) {
    ...
    if (opts.useCache && this.cache.has(cacheKey)) {
      ...
      if (Date.now() - cached.timestamp < this.cacheTTL) {
        ...
        metricsService.logAIMetric({
          ...
          status: 'success',
          metadata: { cacheHit: true, focus: opts.focus },
          userId
        });
    ...
    try {
      ...
      // Behavioral patterns — cognitive distortions (thought records) and urges/
      // cravings. Loaded for full and nervous-system focus, since urges so often
      // ride on a dysregulated state. Gives Huxley awareness of recurring
      // thinking traps and what state/part an urge rode in on.
      if (opts.focus === 'all' || opts.focus === 'nervous_system') {
        context.behavioralPatterns = await this.getBehavioralPatterns(userId, opts);
      }
      ...
      metricsService.logAIMetric({
        ...
        metadata: {
          cacheHit: false,
          focus: opts.focus,
          partsCount: context.ifs?.totalParts || 0,
          journalsCount: context.integrationJournals?.totalJournals || 0,
          connectionsCount: context.potentialConnections?.length || 0
        },
        userId
      });
  ...
  /**
   * Get behavioral patterns — cognitive distortions + urges/cravings
   *
   * Summarizes the two behavioral trackers for AI context:
   * - Cognitive distortions: which thinking traps recur, and whether the user's
   *   belief in distorted thoughts tends to ease after reframing.
   * - Cravings/urges: how often they arise, by category, and what nervous-system
   *   state they ride on (the polyvagal connection the craving tracker invites).
   *
   * Window is opts.recentDays. Returns compact summaries, not raw rows.
   */
  async getBehavioralPatterns(userId, opts) {
    const empty = {
      distortions: { total: 0, topTypes: [], recent: [], beliefEases: null },
      cravings: { total: 0, byCategory: [], stateLinks: [], rodeWaveRate: null, recent: [] },
    };

    try {
      const since = new Date();
      since.setDate(since.getDate() - (opts.recentDays || 90));
      const sinceISO = since.toISOString();

      const [distortionsRes, cravingsRes] = await Promise.all([
        supabase
          .from('cognitive_distortions')
          .select('automatic_thought, distortion_types, balanced_thought, belief_before, belief_after, created_at')
          .eq('user_id', userId)
          .gte('created_at', sinceISO)
          .order('created_at', { ascending: false }),
        supabase
          .from('craving_logs')
          .select('urge_for, urge_category, intensity, ns_state, part_note, rode_the_wave, created_at')
          .eq('user_id', userId)
          .gte('created_at', sinceISO)
          .order('created_at', { ascending: false }),
      ]);

      const distortions = distortionsRes.data || [];
      ...
      const beliefPairs = distortions.filter(d => d.belief_before != null && d.belief_after != null);
      const beliefEases = beliefPairs.length
        ? Math.round(
            (beliefPairs.reduce((s, d) => s + (d.belief_before - d.belief_after), 0) / beliefPairs.length) * 10
          ) / 10
        : null;
      ...
      cravings.forEach(c => {
        ...
        if (c.ns_state) stateCounts[c.ns_state] = (stateCounts[c.ns_state] || 0) + 1;
      });
      const byCategory = Object.entries(catCounts)
        .map(([category, count]) => ({ category, count }))
        .sort((a, b) => b.count - a.count);
      const stateLinks = Object.entries(stateCounts)
        .map(([state, count]) => ({ state, count }))
        .sort((a, b) => b.count - a.count);

      const noted = cravings.filter(c => c.rode_the_wave != null);
      const rodeWaveRate = noted.length
        ? Math.round((noted.filter(c => c.rode_the_wave === true).length / noted.length) * 100) / 100
        : null;

      return {
        distortions: {
          total: distortions.length,
          topTypes,
          beliefEases,
          recent: distortions.slice(0, 3).map(d => ({
            thought: d.automatic_thought,
            types: d.distortion_types || [],
            reframe: d.balanced_thought || null,
            date: d.created_at,
          })),
        },
        cravings: {
          ...
          stateLinks,
          rodeWaveRate,
          recent: cravings.slice(0, 3).map(c => ({
            urgeFor: c.urge_for,
            category: c.urge_category,
            intensity: c.intensity,
            nsState: c.ns_state || null,
            partNote: c.part_note || null,
            rodeTheWave: c.rode_the_wave,
            date: c.created_at,
          })),
        },
      };
    } catch (error) {
      console.error('[MasterContext] Error loading behavioral patterns:', error);
      return empty;
    }
  }
