...
<style>
  @page {
    ...
    /* Extra bottom margin reserves the band the fixed footer/glyph sit in,
       so content never collides with them on any page. */
  ...
  * { box-sizing: border-box; }
  ...
  /* Footer row: brand mark (left) + glyph (right), pinned to the bottom of
     EVERY printed page via fixed positioning. In paged media a position:fixed
     box repeats in the page margin on each page, so the glyph always prints at
     the true page bottom — including on multi-page worksheets, where any torn
     page is then independently scannable. The @page bottom margin reserves
     room so content never overlaps this band. */
  .footer {
    position: fixed;
    bottom: 0;
    left: 0;
  ...
  /* Glyph: small framed text mark. Sized + framed for reliable detection by
     Claude vision (paperScanService.detectGlyph). */
  .glyph {
