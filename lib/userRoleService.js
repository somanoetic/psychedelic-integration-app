...
class UserRoleService {
  ...
  async requestTherapistVerification(verificationData) {
    try {
      ...
      // Flip the existing user_roles row to therapist/unverified.
      // The handle_new_user trigger guarantees every auth user has a row,
      // so this is always an UPDATE, never an INSERT. (An earlier upsert()
      // here defaulted to the primary key for conflict detection and tripped
      // the UNIQUE(user_id) constraint.) The license_* fields are not set —
      // post-ADR-009 the contributor application no longer collects them.
