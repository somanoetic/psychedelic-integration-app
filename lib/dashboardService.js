 ...
 * lastDistortion, lastCraving } with nulls on failure. These map to the Track
...
export async function fetchDashboardData() {
  ...
  const [nsCheckin, habitProgress, glimmerCount, lastTrigger, lastParts, lastDistortion, lastCraving] =
    await Promise.all([
      ...
      fetchLastDistortion(userId),
      fetchLastCraving(userId),
    ]);

  return { nsCheckin, habitProgress, glimmerCount, lastTrigger, lastParts, lastDistortion, lastCraving };
}
...
async function fetchLastDistortion(userId) {
  try {
    const { data } = await supabase
...
async function fetchLastCraving(userId) {
  try {
    const { data } = await supabase
      .from('craving_logs')
      .select('created_at')
...
export async function fetchTrackHistory(limitPerSource = 25) {
  ...
  const [ns, glimmers, triggers, parts, distortions, cravings] = await Promise.all([
    ...
    fetchDistortionEntries(userId, limitPerSource),
    fetchCravingEntries(userId, limitPerSource),
  ]);

  return [...ns, ...glimmers, ...triggers, ...parts, ...distortions, ...cravings].sort(
    (a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
  );
}

const DISTORTION_TYPE_LABELS = {
...
async function fetchDistortionEntries(userId, limit) {
  try {
    const { data } = await supabase
      .from('cognitive_distortions')
      .select('id, automatic_thought, distortion_types, balanced_thought, created_at')
      .eq('user_id', userId)
      .order('created_at', { ascending: false })
      .limit(limit);
    return (data || []).map((row) => ({
      id: `distortion_${row.id}`,
      kind: 'distortion',
      createdAt: row.created_at,
      title: row.automatic_thought || 'A thought record',
      subtitle: (row.distortion_types || [])
        .map((t) => DISTORTION_TYPE_LABELS[t] || t)
        .join(', ') || null,
      intensity: null,
    }));
  } catch {
...
async function fetchCravingEntries(userId, limit) {
  try {
    const { data } = await supabase
      .from('craving_logs')
      .select('id, urge_for, urge_category, intensity, rode_the_wave, created_at')
      .eq('user_id', userId)
      .order('created_at', { ascending: false })
      .limit(limit);
    return (data || []).map((row) => ({
      ...
      kind: 'craving',
      createdAt: row.created_at,
      title: row.urge_for || 'An urge',
      subtitle: row.rode_the_wave === true ? 'Rode it out' : row.urge_category ? capitalize(row.urge_category) : null,
      intensity: row.intensity ?? null,
    }));
  } catch {
    return [];
