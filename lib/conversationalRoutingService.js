import { callClaude } from './claudeAPI';
import huxleyKnowledgeBase from './huxleyKnowledgeBase';
import metricsService from './metricsService';
import { MODELS } from './aiModels';
import { supabase } from './supabase';

/**
 * Conversational Routing Service
 *
 * Interprets user intent and routes to appropriate therapeutic features.
 * Enhanced with clinical protocols from huxleyKnowledgeBase.
 *
 * **Core Functions:**
 * 1. Crisis Detection (HIGHEST priority) - Immediate routing to safety
 * 2. Scenario Detection - Identifies 22 therapeutic scenarios (inner critic, shame, etc.)
 * 3. Intent Classification - Routes to appropriate AI service
 *
 * **Route Map:**
 * - triggered → Triggered Support (crisis/overwhelm)
 * - ifs_chat → IFS AI Service (parts work)
 * - nervous_system_mapping → Polyvagal AI Service
 * - daily_journal → Daily Journal AI
 * - post_session_journal → Integration (Huxley)
 * - triggers_glimmers → Triggers & Glimmers AI
 * - regulating_resources → Regulating Resources AI
 * - core_beliefs → Core Beliefs AI
 * - education → Education content
 * - exercises → Exercise library
 *
 * **Safety Protocol:**
 * - Crisis keywords detected → IMMEDIATE routing (no delay, no questions)
 * - Suicidal ideation, self-harm → triggered_support + crisis resources
 * - Never continue regular conversation if active crisis detected
 *
 * @class
 */
export class ConversationalRoutingService {
  constructor() {
    this.conversationHistory = [];
    this.isOnline = true;
    this.knowledgeBase = huxleyKnowledgeBase;
    // Cross-session memory, loaded from therapeutic_context on login.
    // Before this existed the front-door chat had NO memory at all — it would
    // tell users it had no record of previous conversations while the
    // specialized modes could see their themes and parts. See loadUserContext.
    this.userContext = null;
    // The row id for this user's therapeutic_context, once known (loaded or
    // created). Lets _saveUserContext update-in-place instead of inserting a
    // duplicate row every save, same pattern as huxleyService.persistedContextId.
    this.userContextRowId = null;
    // Set by loadUserContext so later saves know who to write to — routeMessage
    // is never called with a userId, only loadUserContext is (from App.js on
    // login), so this is the only place that id is available.
    this.userId = null;
  }

  /**
   * Load this user's cross-session memory (themes, parts, significant events,
   * last-session recap) from therapeutic_context.
   *
   * Call once on login, alongside huxleyService.initialize(). Failure is
   * non-fatal: the chat still works, just without continuity.
   *
   * @param {string} userId
   */
  async loadUserContext(userId) {
    if (!userId) return;
    this.userId = userId;
    // Tracked so _updateAndSaveUserContext can await this in-flight load
    // instead of merging against a not-yet-populated userContext — otherwise
    // a message sent in the few hundred ms before this resolves would merge
    // against {} and write back a truncated major_events, silently dropping
    // events the specialized modes had already saved for this user.
    this._loadPromise = this._doLoadUserContext(userId);
    return this._loadPromise;
  }

  async _doLoadUserContext(userId) {
    try {
      const { data, error } = await supabase
        .from('therapeutic_context')
        .select('id, themes, parts, major_events, session_summary, last_session_at')
        .eq('user_id', userId)
        .single();

      // PGRST116 = no row yet (new user). Not an error worth logging.
      if (error && error.code !== 'PGRST116') {
        console.warn('[Routing] Could not load user context:', error.message);
        return;
      }
      if (data) {
        this.userContext = data;
        this.userContextRowId = data.id;
        console.log('[Routing] Loaded user context:', {
          themes: data.themes?.length || 0,
          parts: data.parts?.length || 0,
          events: data.major_events?.length || 0,
          hasSummary: !!data.session_summary,
        });
      }
    } catch (err) {
      console.warn('[Routing] Could not load user context:', err.message);
    }
  }

  /**
   * Clear loaded memory. Call on logout so a second account on the same device
   * never sees the first user's context.
   */
  clearUserContext() {
    this.userContext = null;
    this.userContextRowId = null;
    this.userId = null;
    this._loadPromise = null;
  }

  /**
   * Render loaded memory as a prompt block.
   *
   * IMPORTANT — placement: this goes in the USER turn, never the `system` block.
   * The system block carries cache_control:ephemeral, and per-user content there
   * would bust the cached prefix on every user. Same reasoning as the volatile
   * tail in huxleyService, and the same pattern already used for `protocols`.
   *
   * @returns {string} '' when there is nothing worth injecting
   */
  buildUserContextBlock() {
    const c = this.userContext;
    if (!c) return '';

    const lines = [];

    if (c.session_summary) {
      const when = this._describeRecency(c.last_session_at);
      lines.push(`WHERE YOU LEFT OFF${when ? ` (${when})` : ''}: ${c.session_summary}`);
    }

    if (Array.isArray(c.themes) && c.themes.length > 0) {
      lines.push(`ONGOING THEMES: ${c.themes.slice(-6).join('; ')}`);
    }

    if (Array.isArray(c.parts) && c.parts.length > 0) {
      const names = c.parts.map(p => p?.name).filter(Boolean).slice(-6);
      if (names.length) lines.push(`PARTS THEY KNOW: ${names.join(', ')}`);
    }

    if (Array.isArray(c.major_events) && c.major_events.length > 0) {
      const labels = c.major_events.map(e => e?.label).filter(Boolean).slice(-5);
      if (labels.length) lines.push(`SIGNIFICANT IN THEIR LIFE: ${labels.join('; ')}`);
    }

    if (lines.length === 0) return '';

    return `---
WHAT YOU REMEMBER ABOUT THIS PERSON (from previous sessions):
${lines.join('\n')}

Use this for continuity and tone. You MAY briefly reference something here if it
fits naturally, then follow their lead — if they don't pick it up, drop it. Don't
recite this back, don't work through it as a list, and don't imply you remember
more detail than is written here. If they ask what you remember, answer from this
honestly and plainly — it is a summary, not a transcript.`;
  }

  /**
   * Human-friendly recency, so continuity is pitched correctly (picking up a
   * thread from yesterday reads differently from one after two months).
   * @param {string|null} iso
   * @returns {string}
   */
  _describeRecency(iso) {
    if (!iso) return '';
    const then = new Date(iso).getTime();
    if (Number.isNaN(then)) return '';
    const days = Math.floor((Date.now() - then) / 86400000);
    if (days <= 0) return 'earlier today';
    if (days === 1) return 'yesterday';
    if (days < 7) return `${days} days ago`;
    if (days < 14) return 'about a week ago';
    if (days < 60) return `${Math.round(days / 7)} weeks ago`;
    return `${Math.round(days / 30)} months ago`;
  }

  /**
   * Split Claude's raw reply into the visible message and the parsed
   * ---THERAPEUTIC_DATA--- tail (memory extraction), if present. Mirrors
   * huxleyService._parseResponse. Must run BEFORE the ROUTE: regex in
   * sendMessage() so route parsing only ever sees the clean display text.
   *
   * @param {string} rawResponse
   * @returns {{displayMessage: string, therapeuticData: object|null}}
   */
  _parseMemoryExtraction(rawResponse) {
    const marker = '---THERAPEUTIC_DATA---';
    const markerIndex = rawResponse.indexOf(marker);
    if (markerIndex === -1) {
      return { displayMessage: rawResponse.trim(), therapeuticData: null };
    }

    const displayMessage = rawResponse.substring(0, markerIndex).trim();
    const jsonString = rawResponse.substring(markerIndex + marker.length).trim();

    try {
      return { displayMessage, therapeuticData: JSON.parse(jsonString) };
    } catch (e) {
      const jsonMatch = jsonString.match(/\{[\s\S]*\}/);
      if (jsonMatch) {
        try {
          return { displayMessage, therapeuticData: JSON.parse(jsonMatch[0]) };
        } catch (e2) {
          console.warn('[Routing] Could not parse memory extraction:', e2.message);
        }
      }
      return { displayMessage, therapeuticData: null };
    }
  }

  /**
   * Merge this turn's extracted majorEvents/sessionSummary into the in-memory
   * userContext and persist to therapeutic_context. This is the write
   * counterpart to loadUserContext()/buildUserContextBlock() — without it the
   * front-door chat could only ever read memory written by the specialized
   * modes, never capture anything said here (e.g. a breakup mentioned only in
   * main chat, never in IFS/attachment/etc).
   *
   * Merge logic (dedup on label, FIFO cap 15, summary cap 600 chars, coarse-
   * grain enforced by rebuilding fields rather than spreading) intentionally
   * mirrors huxleyService._updateTherapeuticState so the two write paths that
   * share one table behave identically.
   *
   * @param {object} data - parsed therapeuticData from _parseMemoryExtraction
   */
  async _updateAndSaveUserContext(data) {
    if (!this.userId) return;

    // Don't merge against a not-yet-loaded userContext — if loadUserContext()
    // is still in flight, wait for it first. Otherwise a message sent in the
    // window before it resolves would merge against {} and write back a
    // truncated major_events, silently dropping events already saved by the
    // specialized modes.
    if (this._loadPromise) {
      await this._loadPromise.catch(() => {});
    }

    const current = this.userContext || {
      themes: [], parts: [], major_events: [], session_summary: null, last_session_at: null,
    };
    const majorEvents = Array.isArray(current.major_events) ? [...current.major_events] : [];

    if (Array.isArray(data.majorEvents)) {
      const existing = new Set(majorEvents.map(e => (e?.label || '').toLowerCase()));
      for (const raw of data.majorEvents) {
        const label = typeof raw === 'string' ? raw : raw?.label;
        if (!label || typeof label !== 'string') continue;
        const trimmed = label.trim();
        if (!trimmed || existing.has(trimmed.toLowerCase())) continue;
        majorEvents.push({
          label: trimmed.slice(0, 120),
          surfaced_at: new Date().toISOString().slice(0, 10),
          mode: 'main_chat',
        });
        existing.add(trimmed.toLowerCase());
      }
      if (majorEvents.length > 15) majorEvents.splice(0, majorEvents.length - 15);
    }

    let sessionSummary = current.session_summary || null;
    let lastSessionAt = current.last_session_at || null;
    if (typeof data.sessionSummary === 'string' && data.sessionSummary.trim()) {
      sessionSummary = data.sessionSummary.trim().slice(0, 600);
      lastSessionAt = new Date().toISOString();
    }

    const next = { ...current, major_events: majorEvents, session_summary: sessionSummary, last_session_at: lastSessionAt };
    this.userContext = next;

    // Deliberately omit themes/parts: this write path never touches them
    // (that's the specialized modes' job — huxleyService._savePersistedContext).
    // On UPDATE, Supabase upsert only SETs the columns given, so leaving them
    // out means they're untouched here. On INSERT (first-ever row for this
    // user), they fall back to their column defaults ('[]'), which is correct
    // for a genuinely new user regardless of which write path creates the row.
    const row = {
      user_id: this.userId,
      major_events: majorEvents,
      session_summary: sessionSummary,
      last_session_at: lastSessionAt,
      updated_at: new Date().toISOString(),
    };

    if (this.userContextRowId) {
      const { error } = await supabase
        .from('therapeutic_context')
        .update(row)
        .eq('id', this.userContextRowId);
      if (error) throw error;
      return;
    }

    // upsert (not insert): therapeutic_context has a UNIQUE(user_id) constraint,
    // and this row may not have existed yet if the user's first-ever turn was in
    // main chat rather than a specialized mode. A plain insert would throw on a
    // unique-violation if huxleyService's write path created the row first.
    const { data: upserted, error } = await supabase
      .from('therapeutic_context')
      .upsert(row, { onConflict: 'user_id' })
      .select('id')
      .single();
    if (error) throw error;
    if (upserted?.id) this.userContextRowId = upserted.id;
  }

  getSystemPrompt() {
    return `You are Huxley, a warm and compassionate guide helping someone navigate their therapeutic journey.

## YOUR ROLE

You help users find the right tool or practice for what they're experiencing right now. You do this through brief, friendly conversation.

## AVAILABLE FEATURES

You can guide users to these features based on what they need:

1. **Triggered Support** - For immediate crisis/dysregulation
   - Keywords: triggered, overwhelmed, panic, crisis, help now, can't breathe, freaking out
   - Route: triggered_support

2. **Daily Journal** - General-purpose journaling and reflection
   - Keywords: journal, write, reflect, process thoughts, what happened today
   - Route: daily_journal

3. **Process Experience** - For processing non-ordinary consciousness (psychedelic, breathwork, meditation)
   - Keywords: psychedelic, mushrooms, ketamine, MDMA, LSD, ceremony, journey, trip, breathwork, deep meditation, altered state
   - Route: post_session_journal

4. **Integration** - For ongoing integration work after experiences
   - Keywords: integrate, integration, make sense of, understand experience, after journey
   - Route: post_session_journal

5. **Explore Parts (IFS)** - Internal Family Systems parts work
   - Keywords: parts, inner critic, IFS, voices inside, conflicted, part of me
   - Route: ifs_chat

6. **Nervous System** - Explore polyvagal states and body responses
   - Keywords: nervous system, body sensations, fight/flight, shutdown, vagus nerve, polyvagal, dysregulated
   - Route: nervous_system_mapping

7. **Triggers & Glimmers** - Map what dysregulates and regulates you
   - Keywords: triggers, what sets me off, what calms me, glimmers, regulate
   - Route: triggers_glimmers

8. **Regulating Resources** - Build personal regulation toolkit
   - Keywords: coping skills, self-care, regulation practices, what helps, resources
   - Route: regulating_resources

9. **Core Beliefs** - Explore limiting beliefs
   - Keywords: beliefs, not good enough, unworthy, core beliefs, schemas
   - Route: core_beliefs

10. **Learn/Education** - Psychoeducation content
    - Keywords: learn, teach me, education, understand theory, what is IFS, what is polyvagal
    - Route: education

11. **Exercises** - Structured practices and tools
    - Keywords: exercises, practices, techniques, breathing, grounding
    - Route: exercises

## YOUR APPROACH

**First Message:**
- Warm greeting: "Hey there! What's going on for you today?"
- OR if they seem urgent: "I'm here. What do you need right now?"

**Understanding Intent:**
- Ask 1-2 brief clarifying questions if needed
- Listen for emotional tone and urgency
- Match their energy (calm if they're calm, grounded if they're distressed)

**Routing:**
- When you're confident about where to route them, say:
  "Let me take you to [feature name]."
- Then output ONLY the route code (e.g., "ROUTE: daily_journal")

**Crisis Detection:**
- If someone is clearly in crisis (triggered, panicking, overwhelmed), route immediately to triggered_support
- Don't ask questions - get them help fast

## RESPONSE STYLE

- **Brief**: 1-3 sentences max
- **Warm**: Use their name if you know it, show genuine care
- **Clear**: No jargon unless they use it first
- **Responsive**: Match their energy and urgency
- **Non-judgmental**: All experiences and needs are valid

## EXAMPLES

**Example 1: Triggered**
User: "I'm freaking out"
You: "I'm here with you. Let me get you some support right now. ROUTE: triggered_support"

**Example 2: Journal**
User: "I just want to write about my day"
You: "Perfect. Let's open up a space for you to journal. ROUTE: daily_journal"

**Example 3: Needs Clarification**
User: "I had a weird experience"
You: "I'm listening. Was it a psychedelic journey, or something else?"
[Wait for response, then route appropriately]

**Example 4: Parts Work**
User: "There's this voice in my head that won't stop criticizing me"
You: "That sounds like a part of you. Let's explore that together. ROUTE: ifs_chat"

## IMPORTANT

- When you output a route, it MUST be in this exact format: "ROUTE: route_name"
- Only output route codes listed above
- If truly unclear after 2-3 exchanges, default to: "ROUTE: daily_journal"
- NEVER lecture or explain features - just listen and route

## MEMORY ACROSS SESSIONS

You remember people between conversations. When there is anything to remember, it
arrives as a "WHAT YOU REMEMBER ABOUT THIS PERSON" block appended to their latest
message — ongoing themes, parts they've met, significant things in their life, and
a recap of where you left off.

- If that block is present, do NOT say you have no memory of previous conversations.
  If they ask what you remember, answer from it plainly and briefly.
- It is a summary, not a transcript. You will not have the wording of what was said,
  so never imply you recall more detail than is written there.
- If there is no such block, this is a fresh start — say so simply and warmly rather
  than claiming a general inability to remember.

## CLINICAL VOICE PRINCIPLES

- Warm but not saccharine - genuine care without performative positivity
- Curious, not knowing - ask before assuming
- Follow, don't lead - user's experience guides the conversation
- Brief and clear - no jargon unless they use it first
- Connect to body when possible - "Where do you feel that?"

## CRISIS DETECTION (HIGHEST PRIORITY)

If you detect ANY of these, route IMMEDIATELY to triggered_support:
- Suicidal ideation: "want to die", "end my life", "kill myself", "don't want to be here"
- Active self-harm: "hurting myself", "cutting"
- Severe panic: "can't breathe", "dying", "heart attack"

For crisis, say: "I hear you. That sounds really hard. Let me connect you with support right now. ROUTE: triggered_support"

## FORMATTING
- Respond in plain text only. NEVER use markdown formatting (no **, no *, no #).
- Write naturally as spoken conversation.

${this._getMemoryExtractionInstructions()}`;
  }

  /**
   * Structured-output instructions for cross-session memory capture. This is
   * the WRITE side of the front-door chat's memory — buildUserContextBlock()
   * is the read side. Without this, anything said here (a breakup, a job
   * loss, general venting) was never captured: only the specialized modes
   * (IFS, attachment, etc.) extracted and saved therapeutic_context. Mirrors
   * huxleyService._getStructuredOutputInstructions(), but scoped down to just
   * majorEvents + sessionSummary — themes/parts/NS state stay the specialized
   * modes' job so this screen doesn't compete to write them.
   */
  _getMemoryExtractionInstructions() {
    return `## MEMORY CAPTURE (REQUIRED)

After your conversational response (including after any ROUTE: line), ALWAYS append on a new line:

---THERAPEUTIC_DATA---
{
  "majorEvents": [{"label": "short neutral label"}],
  "sessionSummary": "2-3 sentence recap of where this conversation has got to, or null"
}

RULES:
- The user NEVER sees anything after ---THERAPEUTIC_DATA---. The app strips it.
- majorEvents: Significant things happening in this person's LIFE that you would look tone-deaf for having forgotten next time — a death, a diagnosis, a birth, a breakup, job loss, a trauma that surfaced. NOT every difficult feeling. Bar: would it be jarring to open breezily next session without knowing this?
- majorEvents is a LABEL ONLY — a short neutral phrase like "breakup" or "layoff / job loss". Deliberately coarse. Do NOT include detail, quotes, circumstances, names of other people, or clinical interpretation. Omit or use [] on turns where nothing significant surfaced, which is most turns.
- sessionSummary: A rolling 2-3 sentence recap of where THIS conversation has got to. Rewrite it fresh each turn (NOT delta — send the whole current summary). Plain, factual, no interpretation.
- Always include the block, even if all values are null/empty.`;
  }

  /**
   * Get enhanced system prompt with scenario-specific protocols
   *
   * Takes base routing prompt and injects relevant clinical protocols
   * based on detected scenarios in user's message.
   *
   * **How it works:**
   * 1. Detects scenarios from user message (using huxleyKnowledgeBase)
   * 2. Gets top 2 matching scenarios
   * 3. Injects their clinical protocols into system prompt
   * 4. Returns enhanced prompt for Claude API
   *
   * **Example:**
   * User: "I hate myself, I'm so stupid"
   * → Detects 'inner_critic' scenario
   * → Injects INNER_CRITIC_PROTOCOL
   * → Claude knows to externalize as part, explore protective function
   *
   * @param {string} userMessage - User's message
   * @returns {string} Enhanced system prompt with scenario protocols
   *
   * @example
   * const prompt = this.getEnhancedSystemPrompt("I can't stop thinking, always in my head");
   * // Detects 'analyzer' scenario
   * // Returns base prompt + analyzer protocol
   */
  getEnhancedSystemPrompt(userMessage) {
    const basePrompt = this.getSystemPrompt();

    // Detect scenarios from user message
    const detectedScenarios = this.knowledgeBase.detectScenarios(userMessage);

    if (detectedScenarios.length === 0) {
      return basePrompt;
    }

    // Add relevant protocols to prompt
    let enhancedPrompt = basePrompt;
    enhancedPrompt += '\n\n## DETECTED CONTEXT - USE THESE PROTOCOLS:\n';

    // Add top 2 matching protocols
    const topScenarios = detectedScenarios.slice(0, 2);
    for (const scenario of topScenarios) {
      const protocol = this.knowledgeBase.getProtocol(scenario.key);
      if (protocol) {
        enhancedPrompt += '\n' + protocol;
      }
    }

    return enhancedPrompt;
  }

  /**
   * Return ONLY the per-message scenario protocols (the volatile part of the
   * enhanced prompt), without the static base. Used by `getAIResponse` so the
   * static base can be sent as a cached `system` block while the protocols ride
   * in the user message — keeping the cached prefix byte-stable across turns.
   *
   * @param {string} userMessage - User's message
   * @returns {string} Protocol block, or '' if no scenarios detected
   */
  getDetectedProtocols(userMessage) {
    const detectedScenarios = this.knowledgeBase.detectScenarios(userMessage);
    if (detectedScenarios.length === 0) return '';

    const lines = ['## DETECTED CONTEXT - USE THESE PROTOCOLS:'];
    for (const scenario of detectedScenarios.slice(0, 2)) {
      const protocol = this.knowledgeBase.getProtocol(scenario.key);
      if (protocol) lines.push(protocol);
    }
    // If every detected scenario lacked a protocol, emit nothing.
    return lines.length > 1 ? lines.join('\n') : '';
  }

  /**
   * Send message and get routing decision
   *
   * **Main entry point for routing logic.**
   *
   * **Process:**
   * 1. Calls Claude API with enhanced prompt (includes scenario protocols)
   * 2. Claude responds with conversational message + route directive
   * 3. Extracts route from response (looks for "ROUTE: route_name")
   * 4. Returns clean message + route
   * 5. If API fails, falls back to rule-based keyword matching
   *
   * **Route Directive Format:**
   * Claude includes in response: "ROUTE: daily_journal"
   * This gets extracted and the directive removed from visible message.
   *
   * @param {string} userMessage - User's message
   * @returns {Promise<object>} Response with message and route
   * @returns {boolean} return.success - Success status
   * @returns {string} return.message - Conversational response to user
   * @returns {string|null} return.route - Route code (or null if continuing conversation)
   * @returns {boolean} return.isAI - true if Claude API, false if fallback
   *
   * @example
   * const result = await sendMessage("I'm freaking out");
   * // Returns:
   * {
   *   success: true,
   *   message: "I'm here with you. Let me get you some support right now.",
   *   route: "triggered_support",
   *   isAI: true
   * }
   */
  async sendMessage(userMessage, userId = null) {
    const startTime = Date.now();

    try {
      const aiResponse = await this.getAIResponse(userMessage, userId);

      // Check if AI provided a route
      const routeMatch = aiResponse.match(/ROUTE:\s*(\w+)/);

      if (routeMatch) {
        const route = routeMatch[1];
        // Remove route directive from visible message
        const cleanMessage = aiResponse.replace(/ROUTE:\s*\w+/, '').trim();

        // Log routing decision (SECURITY: no user input text - only metadata)
        metricsService.logRoutingDecision({
          inputLength: userMessage?.length || 0,
          detectedIntents: [], // AI-based routing does not use keyword detection
          selectedRoute: route,
          confidence: 0.85, // AI-based routing has higher confidence
          metadata: { method: 'ai', model: MODELS.PRIMARY },
          userId
        });

        return {
          success: true,
          message: cleanMessage,
          route: route,
          isAI: true
        };
      }

      return {
        success: true,
        message: aiResponse,
        route: null,
        isAI: true
      };
    } catch (error) {
      console.error('Routing AI Error:', error);

      // Log error
      metricsService.logError({
        serviceName: 'conversationalRouting',
        operation: 'sendMessage',
        errorType: error.name || 'Error',
        errorMessage: error.message,
        stackTrace: error.stack,
        userId
      });

      // Fallback to rule-based routing
      const fallbackRoute = this.getFallbackRoute(userMessage);

      if (fallbackRoute) {
        // SECURITY: no user input text logged - only metadata
        metricsService.logRoutingDecision({
          inputLength: userMessage?.length || 0,
          detectedIntents: [fallbackRoute], // Log which keyword-based route was matched
          selectedRoute: fallbackRoute,
          confidence: 0.6, // Rule-based routing has lower confidence
          metadata: { method: 'fallback', reason: 'api_error' },
          userId
        });
      }

      return {
        success: true,
        message: this.getFallbackResponse(userMessage),
        route: fallbackRoute,
        isAI: false
      };
    }
  }

  // Alias for backward compatibility
  async routeMessage(userMessage) {
    const result = await this.sendMessage(userMessage);

    // Map route codes to navigation screen names
    const routeMap = {
      'triggered_support': 'TriggeredSupport',
      'daily_journal': 'Journal',
      'post_session_journal': 'ExperienceMapping',
      'ifs_chat': 'IFSChat',
      'nervous_system_mapping': 'NervousSystemMapping',
      'triggers_glimmers': 'TriggersGlimmers',
      'regulating_resources': 'RegulatingResources',
      'core_beliefs': 'CoreBeliefs',
      'education': 'Education',
      'exercises': 'ExerciseLibrary',
    };

    const routeLabels = {
      'triggered_support': 'Support Tools',
      'daily_journal': 'Daily Journal',
      'post_session_journal': 'Experience Processing',
      'ifs_chat': 'Parts Work',
      'nervous_system_mapping': 'Nervous System',
      'triggers_glimmers': 'Triggers & Glimmers',
      'regulating_resources': 'Regulating Resources',
      'core_beliefs': 'Core Beliefs',
      'education': 'Learning',
      'exercises': 'Exercises',
    };

    return {
      message: result.message,
      route: result.route ? routeMap[result.route] : null,
      routeLabel: result.route ? routeLabels[result.route] : null,
    };
  }

  async getAIResponse(userMessage, userId = null) {
    const startTime = Date.now();

    this.conversationHistory.push({
      role: 'user',
      content: userMessage
    });
    this._trimHistory();

    // Prompt caching: send the static base prompt as a cached `system` block,
    // and push the per-message scenario protocols into the user turn instead of
    // appending them to `system` (which would invalidate the cached prefix every
    // turn). Behaviour is unchanged — Claude still sees base + protocols — only
    // the placement differs.
    const systemBlocks = [
      {
        type: 'text',
        text: this.getSystemPrompt(),
        cache_control: { type: 'ephemeral' },
      },
    ];

    const protocols = this.getDetectedProtocols(userMessage);
    // Cross-session memory rides in the user turn for the same reason as
    // protocols: the `system` block is cached, and per-user content there would
    // invalidate the cached prefix every turn.
    const memory = this.buildUserContextBlock();
    const apiMessages = this.conversationHistory.map((m) => ({
      role: m.role,
      content: m.content,
    }));
    if (protocols || memory) {
      const lastIdx = apiMessages.length - 1;
      const extras = [memory, protocols].filter(Boolean).join('\n\n');
      apiMessages[lastIdx] = {
        ...apiMessages[lastIdx],
        content: `${apiMessages[lastIdx].content}\n\n---\n${extras}`,
      };
    }

    try {
      const data = await callClaude({
        model: MODELS.PRIMARY,
        // 300 was sized for the conversational reply alone (1-3 sentences).
        // Raised to fit the ---THERAPEUTIC_DATA--- tail (memory extraction)
        // appended after it — otherwise the JSON block risks truncating
        // mid-response and silently failing to parse.
        max_tokens: 500,
        system: systemBlocks,
        messages: apiMessages,
      });
      const rawResponse = data.content[0].text;
      const { displayMessage: aiMessage, therapeuticData } = this._parseMemoryExtraction(rawResponse);
      const durationMs = Date.now() - startTime;

      // Extract tokens and calculate cost
      const tokens = metricsService.constructor.extractTokens(data);
      const cost = tokens ? metricsService.constructor.calculateCost(tokens) : null;

      // Log success metrics
      metricsService.logAIMetric({
        serviceName: 'conversationalRouting',
        operation: 'getAIResponse',
        durationMs,
        tokens,
        cost,
        status: 'success',
        metadata: { model: MODELS.PRIMARY, max_tokens: 500 },
        userId
      });

      this.conversationHistory.push({
        role: 'assistant',
        content: aiMessage
      });
      this._trimHistory();

      // Fire-and-forget: merge + persist any memory extracted this turn. Must
      // never block or fail the visible reply — same posture as huxleyService's
      // checkpoint save.
      if (therapeuticData) {
        this._updateAndSaveUserContext(therapeuticData).catch(e =>
          console.warn('[Routing] Could not save memory extraction:', e.message)
        );
      }

      return aiMessage;
    } catch (error) {
      const durationMs = Date.now() - startTime;

      // Log error metrics
      metricsService.logAIMetric({
        serviceName: 'conversationalRouting',
        operation: 'getAIResponse',
        durationMs,
        status: 'error',
        metadata: { error: error.message },
        userId
      });

      throw error;
    }
  }

  /**
   * Get fallback response when Claude API unavailable (offline mode)
   *
   * **Uses simple keyword matching** to generate appropriate response.
   * Not as sophisticated as Claude, but functional for offline usage.
   *
   * **Priority order:**
   * 1. Crisis keywords → "I'm here with you..."
   * 2. Psychedelic keywords → "Let's create space to process..."
   * 3. Parts work keywords → "Let's explore that part..."
   * 4. NS keywords → "Let's check in with your nervous system..."
   * 5. Journal keywords → "Let's open a space for you to journal..."
   * 6. Default → "I'm listening. Tell me more..."
   *
   * @param {string} userMessage - User's message
   * @returns {string} Fallback response
   * @private
   */
  getFallbackResponse(userMessage) {
    const lower = userMessage.toLowerCase();

    // Crisis keywords
    if (lower.includes('trigger') || lower.includes('panic') || lower.includes('overwhelm') || lower.includes('crisis') || lower.includes('freaking out') || lower.includes('help')) {
      return "I'm here with you. Let me get you some support right now.";
    }

    // Psychedelic keywords
    if (lower.includes('psychedelic') || lower.includes('mushroom') || lower.includes('ketamine') || lower.includes('journey') || lower.includes('trip') || lower.includes('mdma') || lower.includes('lsd') || lower.includes('ceremony') || lower.includes('ayahuasca')) {
      return "Let's create space to process that experience.";
    }

    // Parts work keywords
    if (lower.includes('part') || lower.includes('critic') || lower.includes('voice') || lower.includes('ifs') || lower.includes('inner')) {
      return "Let's explore that part of you.";
    }

    // Nervous system keywords
    if (lower.includes('nervous') || lower.includes('body') || lower.includes('anxious') || lower.includes('anxiety') || lower.includes('calm') || lower.includes('relax')) {
      return "Let's check in with your nervous system.";
    }

    // Journal keywords
    if (lower.includes('journal') || lower.includes('write') || lower.includes('reflect') || lower.includes('thought') || lower.includes('feel')) {
      return "Let's open a space for you to journal.";
    }

    // Learn keywords
    if (lower.includes('learn') || lower.includes('what is') || lower.includes('teach') || lower.includes('understand') || lower.includes('explain')) {
      return "Let me share some helpful information with you.";
    }

    // Exercise/practice keywords
    if (lower.includes('exercise') || lower.includes('practice') || lower.includes('breath') || lower.includes('ground') || lower.includes('meditat')) {
      return "Let me show you some helpful practices.";
    }

    // Greeting responses
    if (lower.includes('hi') || lower.includes('hello') || lower.includes('hey') || lower.includes('good morning') || lower.includes('good evening')) {
      return "Hey there! What's on your mind today?";
    }

    // How are you / checking in
    if (lower.includes('how are you') || lower.includes("what's up") || lower.includes('okay') || lower.includes('doing well') || lower.includes('i\'m good')) {
      return "I'm here for you. What would you like to explore today?";
    }

    // Emotional expressions
    if (lower.includes('sad') || lower.includes('depressed') || lower.includes('down') || lower.includes('lonely') || lower.includes('hurt')) {
      return "I hear you. Would you like to journal about what you're feeling, or would some support tools be helpful?";
    }

    if (lower.includes('angry') || lower.includes('frustrated') || lower.includes('mad') || lower.includes('annoyed')) {
      return "Those feelings are valid. Would you like to explore what's coming up, or try some regulation practices?";
    }

    // Default - more engaging
    return "I'm listening. Tell me more about what's going on for you, and I'll help point you in the right direction.";
  }

  /**
   * Get fallback route when Claude API unavailable (offline mode)
   *
   * **Uses simple keyword matching** to determine route.
   * Not as sophisticated as Claude, but functional for offline usage.
   *
   * **CRITICAL: Crisis detection is first priority.**
   *
   * **Route Priority:**
   * 1. Crisis keywords → triggered_support (HIGHEST PRIORITY)
   * 2. Psychedelic keywords → post_session_journal
   * 3. Parts work keywords → ifs_chat
   * 4. Nervous system keywords → nervous_system_mapping
   * 5. Glimmer keywords → triggers_glimmers
   * 6. Belief keywords → core_beliefs
   * 7. Learn keywords → education
   * 8. Exercise keywords → exercises
   * 9. Journal keywords → daily_journal
   * 10. Default → null (continue conversation)
   *
   * @param {string} userMessage - User's message
   * @returns {string|null} Route code or null if no route
   * @private
   *
   * @example
   * getFallbackRoute("I'm having a panic attack");
   * // Returns: "triggered_support"
   *
   * @example
   * getFallbackRoute("Tell me about IFS");
   * // Returns: "education"
   */
  getFallbackRoute(userMessage) {
    const lower = userMessage.toLowerCase();

    // Simple keyword matching for offline mode
    if (lower.includes('trigger') || lower.includes('panic') || lower.includes('crisis') || lower.includes('freaking out') || lower.includes('overwhelm')) {
      return 'triggered_support';
    }

    if (lower.includes('psychedelic') || lower.includes('mushroom') || lower.includes('ketamine') || lower.includes('journey') || lower.includes('trip') || lower.includes('ceremony') || lower.includes('mdma') || lower.includes('lsd') || lower.includes('ayahuasca')) {
      return 'post_session_journal';
    }

    if (lower.includes('part') || lower.includes('critic') || lower.includes('ifs') || lower.includes('inner voice')) {
      return 'ifs_chat';
    }

    if (lower.includes('nervous') || lower.includes('polyvagal') || lower.includes('dysregulated')) {
      return 'nervous_system_mapping';
    }

    if (lower.includes('glimmer')) {
      return 'triggers_glimmers';
    }

    if (lower.includes('belief') || lower.includes('worthy') || lower.includes('schema') || lower.includes('not good enough')) {
      return 'core_beliefs';
    }

    if (lower.includes('learn') || lower.includes('education') || lower.includes('teach') || lower.includes('what is') || lower.includes('explain')) {
      return 'education';
    }

    if (lower.includes('exercise') || lower.includes('practice') || lower.includes('breathing') || lower.includes('ground') || lower.includes('meditat')) {
      return 'exercises';
    }

    if (lower.includes('anxious') || lower.includes('anxiety') || lower.includes('calm') || lower.includes('relax') || lower.includes('body')) {
      return 'nervous_system_mapping';
    }

    if (lower.includes('journal') || lower.includes('write') || lower.includes('reflect')) {
      return 'daily_journal';
    }

    if (lower.includes('sad') || lower.includes('depressed') || lower.includes('down') || lower.includes('feel')) {
      return 'daily_journal';
    }

    // For greetings and general conversation - no route, just chat
    if (lower.includes('hi') || lower.includes('hello') || lower.includes('hey') || lower.includes('how are you') || lower.includes('okay') || lower.includes('i\'m good')) {
      return null; // No route - continue conversation
    }

    // Default to null (no route) for general conversation
    return null;
  }

  reset() {
    this.conversationHistory = [];
  }

  getConversationHistory() {
    return this.conversationHistory;
  }

  // Seed an assistant-side greeting into history so the router knows the
  // conversation has already opened. Used by the chat screen on first mount,
  // where the visible greeting ("Welcome back.", "Hey there 👋") is rendered
  // directly without going through routeMessage(). Without this seed, the
  // user's first reply would arrive at the router as a cold "turn 1" — and
  // the router would respond with its own opening greeting on top of the
  // visible one. Seeded turns count toward the MAX_HISTORY cap.
  seedAssistantGreeting(text) {
    if (!text || !text.trim()) return;
    // Don't double-seed if the last assistant turn is already this text.
    const last = this.conversationHistory[this.conversationHistory.length - 1];
    if (last && last.role === 'assistant' && last.content === text) return;
    this.conversationHistory.push({ role: 'assistant', content: text });
    this._trimHistory();
  }

  // Cap history so the prompt context doesn't grow unbounded across a long
  // triage chat. We keep the most-recent MAX_HISTORY turns; older context that
  // matters long-term lives in masterContext + therapeutic_context, not here.
  _trimHistory() {
    const MAX_HISTORY = 20;
    if (this.conversationHistory.length > MAX_HISTORY) {
      this.conversationHistory = this.conversationHistory.slice(-MAX_HISTORY);
    }
  }

  // Return the last N turns formatted as a single string, for handing off to
  // huxleyService when a route fires. Lets the destination mode start with
  // some awareness of what the user just said in the router chat — fixes the
  // perceived "Huxley forgot what I said" gap on navigation.
  getRecentContextSummary(turns = 3) {
    if (this.conversationHistory.length === 0) return null;
    const recent = this.conversationHistory.slice(-turns * 2); // 2 = user+assistant per turn
    const lines = recent.map((m) => {
      const speaker = m.role === 'user' ? 'User' : 'Huxley';
      return `${speaker}: ${m.content}`;
    });
    return lines.join('\n');
  }
}

export default new ConversationalRoutingService();
