...
import { MODELS, DEFAULT_MODEL } from './aiModels';
...
class MetricsService {
  constructor() {
    ...
    // Claude pricing (per million tokens). Keys must match MODELS values in aiModels.js.
    // Update when adding/changing models — calculateCost() returns 0 for unknown models.
    this.CLAUDE_PRICING = {
      [MODELS.PRIMARY]: {
   ...
   * @param {string} [model=DEFAULT_MODEL] - Model name (see aiModels.js)
   * @returns {number} Estimated cost in USD
   */
  static calculateCost(tokens, model = DEFAULT_MODEL) {
