...
class PhilosophicalTalkthroughModeHandler extends BaseModeHandler {
  ...
  _getPhaseGuidance() {
    ...
    const scenario = this.selectedTopic?.openingTheme || '';
    const isScenario = this.selectedTopic?.category === 'thought_experiment';

    // Thought experiments must have their scenario laid out before any question
    // makes sense; contemplative themes can open straight into inquiry.
    const openingGuidance = isScenario
      ? `OPENING INQUIRY phase for the thought experiment "${topicTitle}". On your FIRST message, present the scenario clearly and vividly in your own warm voice (you may draw on this text: "${scenario}"), then end with the question it poses or "What's your first honest reaction?" Do not lecture about it or give the "answer." Guiding tensions: ${themes}. After they respond, follow their thread with Socratic questions. If they connect it to a psychedelic experience, follow that.`
      : `OPENING INQUIRY phase for "${topicTitle}". Your role is Socratic companion — ask questions, don't lecture. Begin with the topic's central question. Let the user's first responses guide the direction. Guiding themes: ${themes}. Keep responses to 2-3 sentences, ending with a question. Meet them where they are — if they share a psychedelic experience that connects, follow that thread.`;

    const guidance = {
      opening_inquiry: openingGuidance,
