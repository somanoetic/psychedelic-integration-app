...
class RAGService {
  constructor() {
    this.cache = new Map();
    // Per-stage server timing from the LAST non-cached search, so the caller
    // (huxleyService's [Huxley PERF] line) can show the real auth/embed/rpc
    // split of a device turn without reading Supabase edge logs. Null until a
    // live search runs; a cache hit leaves the previous value untouched.
    // { authMs, embedMs, embedCacheHit, rpcMs, clientMs } — see edge fn timing.
    this.lastServerTiming = null;
  }
  ...
  async search(query, options = {}) {
    ...
    try {
      ...
      // Capture per-stage server timing (auth/embed/rpc) so the [Huxley PERF]
      // line can show where the RAG second actually goes. clientMs is the full
      // client-observed fetch wall-time; the gap (clientMs − auth − embed − rpc)
      // is network/TLS + JSON transfer outside the measured server stages.
      if (data.timing) {
        this.lastServerTiming = { ...data.timing, clientMs: durationMs };
      }
      ...
      if (__DEV__) {
        // Level-1 retrieval visibility: show not just the count/latency but WHAT
        // came back — source, section, similarity score, and a content snippet —
        // so we can see the actual knowledge Huxley is (or isn't) being fed each
        // turn. Filter tuning and the "0 results" question are diagnosed from
        // these lines. Snippet trimmed to keep the log readable.
        const catLabel = categories ? categories.join(',') : 'all';
        const t = data.timing;
        const splitLabel = t
          ? ` | server auth=${t.authMs}ms embed=${t.embedMs}ms` +
            `(${t.embedCacheHit ? 'HIT' : 'MISS'}) rpc=${t.rpcMs}ms` +
            ` net/other=${Math.max(0, durationMs - t.authMs - t.embedMs - t.rpcMs)}ms`
          : '';
        console.log(
          `[RAG] Search: ${results.length} result(s) in ${durationMs}ms ` +
          `| query="${query.substring(0, 80)}" cats=[${catLabel}] threshold=${threshold}` +
          splitLabel
        );
        results.forEach((r, i) => {
          const score = typeof r.similarity === 'number'
            ...
            : '?';
          const source = r.source_document || 'unknown';
          const section = r.section_title || '(no section)';
          const snippet = (r.content || '')
            .replace(/\s+/g, ' ')
            .substring(0, 120);
          console.log(
            `[RAG]   ${i + 1}. [${score}] ${source} / ${section} — "${snippet}${(r.content || '').length > 120 ? '…' : ''}"`
          );
        });
        if (results.length === 0) {
          console.log(
            `[RAG]   (no chunks cleared threshold ${threshold} for cats [${catLabel}] — ` +
            `query may be too vague, threshold too strict, or corpus thin for this category)`
          );
        }
      }
